...
public class ReactorNettyTcpClient<P> implements TcpOperations<P> {
	...
	public static final class MessageHandlerConfiguration<P> {
		...
		final Function<? super ByteBuf, ? extends Collection<Message<P>>> decoder;
		final BiConsumer<? super ByteBuf, ? super Message<P>>             encoder;
		final int                                                         backlog;
	...
	public ListenableFuture<Void> connect(TcpConnectionHandler<P> connectionHandler,
		...
		promise = promise.retryWhen(errors -> errors.scan(1, (p, e) -> p++)
		                                            .flatMap(attempt -> Mono.delayMillis(
				                                            strategy.getTimeToNextAttempt(
						                                            attempt))));
	...
	private static final class MessageHandler<P>
		...
		public Publisher<Void> apply(NettyInbound in, NettyOutbound out) {
			Flux<ByteBuf> inbound = in.receive();

			if(configuration.scheduler != null){
				inbound = inbound.publishOn(configuration.scheduler, configuration
						.backlog);
			}
